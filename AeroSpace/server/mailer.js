...
async function getTransporter() {
  ...
  // 1. If real Gmail/SMTP App Password credentials are provided in .env
  if (user && pass && pass.trim().length >= 8 && pass !== 'your_16_character_app_password') {
    if (!transporter) {
      if (process.env.SMTP_SERVICE === 'gmail' || host === 'smtp.gmail.com' || user.endsWith('@gmail.com')) {
    ...
    return { transport: transporter, mode: 'LIVE_SMTP', sender: DEFAULT_SENDER };
  }

  // 2. Otherwise create / reuse Ethereal Test Account with official sender address
  if (!etherealAccount) {
    try {
  ...
  // 3. Fallback dummy transporter (JSON transport)
...
export async function sendOTPEmail({ to, otp, type = 'LOGIN', name = 'Operator' }) {
  try {
    ...
    const html = buildOtpEmailHtml({ email: to, otp, type, name });
    const text = `AeroSpace 2FA Security Code: ${otp}. Valid for 10 minutes. Sent from ${OFFICIAL_SENDER_EMAIL}. If you did not request this, please ignore.`;
    ...
    console.log(`\n══════════════════════════════════════════════════════`);
    console.log(`🚀 [REAL EMAIL DISPATCH SUCCESS]`);
    console.log(` Sender:      ${OFFICIAL_SENDER_EMAIL}`);
